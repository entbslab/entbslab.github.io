...
### 공지 추가
1. `/admin/` → 로그인 → 왼쪽 **공지사항** → **공지사항 목록**
2. **공지** 목록 위의 **+ 항목 추가** (또는 Add) 클릭 → 제목, 날짜, 상단 고정 여부, 본문, 첨부파일 입력
3. 오른쪽 위 **저장(Save)** → 1~2분 뒤 사이트에 반영

### 순서 바꾸기·삭제
- 목록 항목의 손잡이를 끌어 순서 변경, 항목 메뉴에서 삭제. 공지·소식은 날짜순으로 자동 정렬되므로 순서는 연구진·활동분야에서만 의미가 있습니다.
